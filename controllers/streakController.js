...
const getStats = async (req, res) => {
   ...
   try {
    ...
    console.log('User ID:', req.userId); // ✅ Safe debug log

    let startDate = dayjs(user.createdAt).startOf('day');

    if (user.coupleId) {
      const couple = await Couple.findById(user.coupleId);
      if (couple) startDate = dayjs(couple.createdAt).startOf('day');
    } else {
      return res.status(404).json({ error: 'Couple not linked yet' });
    }
    ...
    uniqueDates.forEach(date => {
      if (!prevDate) {
        ...
        const expectedPrev = dayjs(prevDate).subtract(1, 'day').format('YYYY-MM-DD');
        streak = date === expectedPrev ? streak + 1 : 1;
      }
      highestStreak = Math.max(highestStreak, streak);
      prevDate = date;
    });
...
module.exports = { getStreakStats, getStats };